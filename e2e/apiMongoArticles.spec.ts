...
test("apiMongoArticles", async ({ browserName, request }) => {
  const context = {
    vars: {
  ...
  await test.step("should add an article", async () => {
    const addArticleResponse = await request.post("/api/mongo-articles", {
      multipart: (() => {
  ...
  await test.step("should get articles", async () => {
    const getArticlesResponse = await request.get("/api/mongo-articles");
  ...
  await test.step("should get an article", async () => {
    const getArticleResponse = await request.get(`/api/mongo-articles/${_id}`);
  ...
  await test.step("should edit an article", async () => {
    const editArticleResponse = await request.put(`/api/mongo-articles/${_id}`, {
      multipart: (() => {
        const dto = new FormData();
  ...
  await test.step("should remove an article", async () => {
    const removeArticleResponse = await request.delete(`/api/mongo-articles/${_id}`);
