...
export async function getArticles() {
  return agent(app).get("/api/postgres-articles");
}
...
export async function addArticle(
  ...
  const request = agent(app).post("/api/postgres-articles");
...
export async function getArticle(_id: Article["_id"]) {
  return agent(app).get(`/api/postgres-articles/${_id}`);
}
...
export async function editArticle(
  ...
  const request = agent(app).put(`/api/postgres-articles/${_id}`);
...
export async function removeArticle(_id: Article["_id"]) {
  return agent(app).delete(`/api/postgres-articles/${_id}`);
}
