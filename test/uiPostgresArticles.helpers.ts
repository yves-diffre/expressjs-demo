...
export async function displayArticles() {
  return agent(app).get("/postgres-articles");
}

export async function displayAddArticle() {
  return agent(app).get("/postgres-articles/add-article");
}
...
export async function addArticle(
  ...
  const request = agent(app).post("/postgres-articles/add-article");
...
export async function displayArticle(_id: Article["_id"]) {
  return agent(app).get(`/postgres-articles/${_id}`);
}

export async function displayEditArticle(_id: Article["_id"]) {
  return agent(app).get(`/postgres-articles/${_id}/edit-article`);
}
...
export async function editArticle(
  ...
  const request = agent(app).post(`/postgres-articles/${_id}/edit-article`);
...
export async function removeArticle(_id: Article["_id"]) {
  return agent(app).post(`/postgres-articles/${_id}/remove-article`).redirects(1);
}
